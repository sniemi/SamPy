...
def doit(fn,dp,mt):
  """ JHT: this is the module that analyses an ALFOSC image of
           a Horizontal Slit, and recommends an alignment offset to the
  ...
  messageOut(mt,"\nHorizontal-Slit analysis of file:  "+dp+fn+".fits\n")
  ...
  if grismid.find("pen")==-1: 
     messageOut(mt,"Horizontal-Slit mode:  grism wheel should be Open\n")
     return "File  %s:  Horizontal-Slit mode:  grism wheel should be Open" % fn
  ...
  if slitid.find(" Slit")==-1: 
     messageOut(mt,"Horizontal-Slit mode:  no horizontal slit in aperture wheel\n")
     return "File  %s:  Horizontal-Slit mode:  no horizontal slit in aperture wheel" % fn
  ...
  width=7*slitwidth    # x 7 or else the widest slits go wrong
...
###    b=2*C2/(max-min)
  ...
  iraf.images.imutil.imgets(dp+fn,"CRVAL2")
  ystart=float(iraf.images.imutil.imgets.value)
  messageOut(mt,"\nBinning factors "+str(int(xbin))+" x "+str(int(ybin))+"\n")
  if ybin==1:
    messageOut(mt,"\nSlitpos:   Y-position at CCD center "+str((ystart-1+a+b*(lower+upper)/2))+" according to fit\n")
  else:
    messageOut(mt,"\nSlitpos:   Y-binning is not 1:   set wrongly in hsalign.run ?!\n\n")
