...
# -*- coding: utf-8 -*-
#
# Python version of the betalight script for StanCam, based on the IDL one
#
# The original script for ALFOSC:
# (C) 2007, Ricardo Cárdenes for the Nordic Optical Telescope
# StanCam version edited by Sami-Matias Niemi
# More comments are found in alfosc.py
...
def insertIntoDB(server, user, passwd, db, data, verbose = False):
    ...
    nrows = curs.execute("SELECT * FROM qc_stancam_CCD1")
...
if __name__ == '__main__':
    ...
    #Test for a valid StanCam file name
    ...
    if fname[0] not in au or fname[1] not in au:
        print "Not a valid StanCam file name"
        sys.exit(-1)
