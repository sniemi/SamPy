...
#	self.font = ('Verdana', 8, 'bold')
	...
	self.plotarea_size[0] = 0.80 * self.width
    ...
    def draw(self, graphics, xaxis = None, yaxis = None, expand=1):
	...
	if expand:
	    dx = (p2[0]-p1[0])*0.01
	    dy = (p2[1]-p1[1])*0.01
	xaxis = self._axisInterval(xaxis, p1[0]-dx, p2[0]+dx)
	yaxis = self._axisInterval(yaxis, p1[1]-dy, p2[1]+dy)
...
#    _multiples = [(2., math.log10(2.)), (5., math.log10(5.))]
    _multiples = [(1., math.log10(1.)), (5., math.log10(6.))]
...
if __name__ == '__main__':
    ...
    for i in range(18):
        ...
        data2.append((float(i)*di,
                     2*(math.sin(float(i)*di)-math.cos(float(i)*di))))
    line  = GraphLine(data, color='gray', smooth=0)
    linea = GraphLine(data, color='blue', smooth=1, splinesteps=500)
    lineb = GraphLine(data2, color='red', smooth=0 )

    graphObject = GraphObjects([line, linea, lineb])
