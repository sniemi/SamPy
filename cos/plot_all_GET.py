...
def mylinearregression(x, y, confidence_level=95,
...
if __name__ == '__main__':
    ...
    for i in ran:
        ...
        print 'Plot %i, change %f +/- %f per cent per year, all data. Not plotted.' % (
        c, fit[0] * 356. * 100., tmp[1] * 356. * 100.)
        ...
        print 'Plot %i, change %f +/- %f per cent per year, only on orbit data (%i points). Green dotted line.' % (
        c, fit2[0] * 356. * 100., tmp[1] * 356. * 100., len(all_mjd[msk]))
        ...
        print 'Plot %i, change %f +/- %f per cent per year, all data excluding the two outliers. Magenta dot-dashed.' % (
        c, fit3[0] * 356. * 100., tmp[1] * 356. * 100.)
        ...
        print 'Plot %i, change %f +/- %f per cent per year, ground data excluding the two clear outliers. Red hatched line.' % (
        c, fit4[0] * 356. * 100., tmp[1] * 356. * 100.)
